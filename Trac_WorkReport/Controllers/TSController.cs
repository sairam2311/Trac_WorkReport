...
namespace Trac_WorkReport.Controllers
{
    public class TSController : Controller
    {
        ...
        {
            if (!model.reportViewModels.Any())
            {
               // return View("Error", new CustomErrorViewModel { ErrorMessage = "No report data found." });
            }

            var firstReportViewModel = model.reportViewModels.First();
            var userId = firstReportViewModel.UserId;

            // Fetch the user's timesheets from the repository
            ...
            if (firstReportViewModel.StartDate.HasValue && firstReportViewModel.EndDate.HasValue)
            {
                reports = reports
                    .Where(t => t.ReportDate >= firstReportViewModel.StartDate && t.ReportDate <= firstReportViewModel.EndDate)
                    .ToList();
            }

            // Create a list to hold the transformed timesheet view models
            var timeSheetViewModels = new List<TimeSheet>();

            // Handle the transformation of each report sequentially
            foreach (var report in reports)
            {
                string assignedByName;

                // Fetch the 'AssignedBy' user's name only if it's not "Self Reporting"
                if (report.AssignedBy == "Self Reporting")
                {
                    assignedByName = "Self Reporting";
                }
                else
                {
                    var user = await _userManager.FindByIdAsync(report.AssignedBy);
                    assignedByName = user?.EmployeeName ?? "Unknown";
                }

                // Add the transformed report to the view model list
                timeSheetViewModels.Add(new TimeSheet
                {
                    TSid = report.TSid,
                    Work = report.Work,
            ...
            // Update the first report view model with the transformed timesheets and user details
            firstReportViewModel.TimeSheets = timeSheetViewModels;
            firstReportViewModel.EmployeeName = cuser?.EmployeeName ?? "Unknown";
            firstReportViewModel.EmployeeID = cuser?.EmployeeID ?? "Unknown";

            // Return the modified model to the view
            return View(model);
        }
        ...
        public async Task<IActionResult> UpdateTSOneMOre(Guid ID, string userId)
        {
            ...
            var revrep = _employeeMapRep.GetOfficersID(userId);
            // Handle the case where revrep is null or empty
